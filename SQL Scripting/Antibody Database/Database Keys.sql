...
---This makes sure that all passwords are unique
ALTER TABLE Users
	ADD CONSTRAINT AK_Users_password
	UNIQUE(password)
...
---Make sure that a Panel or PracticeQuestion doesn't use the same citem_id
ALTER TABLE Panels
	ADD CONSTRAINT AK_Panels_citem_id
	UNIQUE(citem_id)
	
ALTER TABLE PracticeQuestions
	ADD CONSTRAINT AK_PracticeQuestions_citem_id
	UNIQUE(citem_id)
